...
static int pivariety_power_on(struct device *dev)
{
	...
	gpiod_set_value_cansleep(pivariety->enable_gpio, 1);
...
static int pivariety_power_off(struct device *dev)
{
	...
	gpiod_set_value_cansleep(pivariety->enable_gpio, 0);
				 ...
				 struct v4l2_mbus_frame_desc *fd)
{
	...
	format = &pivariety->supported_formats[pivariety->current_format_idx];

	fd->entry[0].pixelcode = format->mbus_code;
	fd->entry[0].bus.csi2.dt = format->data_type;
...
static int pivariety_probe(struct i2c_client *client)
{
	...
	pivariety->enable_gpio = devm_gpiod_get_optional(dev, "enable",
							GPIOD_OUT_HIGH);
