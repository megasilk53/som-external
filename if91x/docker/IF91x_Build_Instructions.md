...
To check the version on a running target (module loaded):

```
modinfo infhosted | grep version
```

To inspect the `.ko` file directly on the host:

```
modinfo /path/to/infhosted.ko | grep version
```
